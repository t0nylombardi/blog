...
export default function SuccessPopup({onClose}: SuccessPopupProps) {
  return (
    <>
      ...
      >
        <div
          className="bg-blog-blue px-6 py-4 rounded-lg shadow-lg w-[60rem] flex flex-col relative"
          style={{color: 'var(--text-primary)'}}
        >
          <button onClick={onClose} className="absolute top-2 right-2">
            <X size={30} className="hover:opacity-80 transition-opacity" />
          </button>
          ...
          <p className="text-lg font-medium my-4">The reply is coming. Soon.</p>
        </div>
      </motion.div>
    </>
  )
